...
def ingest_in_sales(session)-> None:
    session.sql(" \
            ...
            from \
            @sales_dwh.source.MY_S3_STAGE/source=IN/format=csv/ \
            (                                                             \
...
def ingest_us_sales(session)-> None:
    session.sql(' \
            ...
            (                                       \
                ...
                metadata$file_last_modified as stg_last_modified\
                from                                \
                    @sales_dwh.source.MY_S3_STAGE/source=US/format=parquet/\
...
def ingest_fr_sales(session)-> None:
    session.sql(' \
        ...
        (                                                       \
            ...
            metadata$file_last_modified as stg_last_modified\
            from                                                \
            @sales_dwh.source.MY_S3_STAGE/source=FR/format=json/\
